...
| `GAP5` | **No change-set model** — edits are immediate, with optimistic concurrency and a change log but no proposal, review or approval | `PLAT1` and `PLAT4` | Initiative 4 for the change-set core (branches, diff, merge with base versions); the review and approval flow follows once roles are enforced |
...
| `GAP12` | **No target state model** — an element has a workflow status and the source tool's lifecycle text, no current state against a target state, no work package to analyse by | `PLAT1` and `PLAT4` | Initiative 4: current and target state on every element and relationship, the Target state page, marked views |
| `GAP13` | **No proposal intake** — a design document cannot be handed to the repository; every element is typed in by hand or imported from a tool | `PLAT1` and `PLAT4` | Initiative 5: the Propose module and the Proposal Template |
