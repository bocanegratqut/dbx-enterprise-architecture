...
| 4   | [4_branches-and-target-state.md](./4_branches-and-target-state.md) | not started (Understanding pending) | Model branches as overlays merged with base versions; current versus target state on every artefact, analysed by work package |
| 5   | [5_propose.md](./5_propose.md) | not started (Understanding pending) | Propose: a document, file or link becomes a branch through an agent that links existing elements, adopts new ones and pushes back when the sources are insufficient; the Proposal Template |
